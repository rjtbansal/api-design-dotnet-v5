...
| `src/middleware/validation.ts` (`validateBody`/`validateParams`/`validateQuery`, Zod schemas) | Data Annotations (`[Required]`, etc.) on request DTOs in `Models/`, e.g. `CreateHabitRequest` | `[ApiController]` automatically checks `ModelState` — populated from `[FromBody]`, `[FromRoute]`, and `[FromQuery]` alike — and short-circuits with `400` before the action runs, for any parameter or DTO property carrying a validation attribute. One built-in mechanism replaces three hand-written Express middleware functions |
